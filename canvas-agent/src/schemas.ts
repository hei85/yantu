...
export const toolNames = [
    ...
    // 影视流水线：调度衍图已有的 Prompt Operation 与生成任务，而不是重新实现影视能力。
...
export const toolInputSchemas = {
    ...
    film_list_operations: z.object({}),
    film_run_operation: z.object({
    ...
    film_wait_task: z.object({ taskId: z.string().min(1), timeoutMs: z.number().optional(), pollMs: z.number().optional() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    film_list_operations: "列出衍图已定义的影视提示词操作（短剧大纲、章节资产提取、角色提取、角色三视图、分镜规划、分镜修复、分镜首帧、分镜视频）及其变量、输出类型与默认链路。开始影视任务前先调用一次，不要自己编造操作名。",
    film_run_operation: "调用一个已有的影视提示词操作：按 operation + variables 发起衍图生成任务（模板由服务端当前启用的版本渲染），可选择立即等待完成。同一镜头重试时必须复用同一个 clientOperationId，避免重复提交与重复计费。",
    film_wait_task: "等待或查询一个已提交的生成任务直到终态，返回状态、进度、错误与产物；超时表示任务仍在进行，可以稍后再次等待，但不要重复提交同一任务。",
};
