...
这个插件把衍图的本地 Canvas Agent MCP 打包给 Codex app 使用，让 Codex 能打开本地画布、读取当前节点、创建内容并触发生成流程。

## 安装

> 衍图尚未上架 Codex 公共插件目录，直接搜索不会显示。请从本仓库自带的 marketplace 安装。
...
```text
请从 https://github.com/ddcat-ai/open-ai-canvas.git 安装衍图 Codex 插件。
...
如果你就在衍图仓库中调试插件，可以直接添加当前仓库。建议使用仓库绝对路径，避免 Codex 从其他工作目录解析失败：
...
1. 新建 Codex 线程后说“打开衍图”。
2. 插件会确认当前仓库的本地画布服务是否已运行；端口被占用时会检查进程归属，不会把其他项目的 `3000` 当作衍图。
