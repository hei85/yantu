...
if (Test-CanvasHttp $healthUrl) {
    Write-Host "衍图画布已经在运行，正在打开..." -ForegroundColor Green
...
if ($occupiedPorts.Count -gt 0) {
    throw "端口 $($occupiedPorts -join ', ') 已被其他程序占用，无法安全启动衍图。"
}
...
Write-Host "正在启动衍图前端和后端，请稍候..." -ForegroundColor Cyan
& $localStarter
...
do {
    ...
    if (Test-CanvasHttp $healthUrl) {
        Write-Host "衍图画布已启动，正在打开浏览器..." -ForegroundColor Green
        Start-Process $canvasUrl
        exit 0
    }
} while ((Get-Date) -lt $deadline)

throw "衍图服务已拉起，但 3 分钟内未通过健康检查。请查看打开的前端、后端窗口。"
