...
func DefaultRuntimePolicy() RuntimePolicySetting {
	value := baseRuntimePolicy()
	if selfUseModeEnabled() {
		applySelfUseMaximums(&value)
	}
	return value
}

// baseRuntimePolicy 是官方默认基线（含部署级环境覆盖）。
func baseRuntimePolicy() RuntimePolicySetting {
...
// applySelfUseMaximums 把单机自用工作站的资源、任务与请求策略提升到上限：
// 本地个人创作不设置账号容量、每日上传量与素材数量限制。
func applySelfUseMaximums(value *RuntimePolicySetting) {
...
// selfUseModeEnabled 读取单机自用开关：CANVAS_SELF_USE_MODE=1 时本地工作站不再限制容量与配额。
func selfUseModeEnabled() bool {
	return envInt("CANVAS_SELF_USE_MODE", 0) == 1
}
