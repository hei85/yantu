...
// 本地单用户工作站已按要求下线充值、支付与套餐模块，官方支付插件包未随当前构建安装。
// 这些断言在没有对应插件/provider 时没有可测对象，跳过而不是误报成回归。
func skipWhenPaymentPluginsAbsent(t *testing.T, center *pluginRuntime, pluginIDs ...string) {
	t.Helper()
	...
	for _, id := range pluginIDs {
		if !installed[id] {
			t.Skipf("payment plugin %s is not installed in this build", id)
		}
	}
}

func skipWhenPaymentProvidersAbsent(t *testing.T, center *pluginRuntime, providerIDs ...string) {
	t.Helper()
	...
	for _, id := range providerIDs {
		...
		t.Skipf("payment provider %s is not installed in this build", id)
	}
}
...
func TestSystemPaymentPluginsRegisterRPCProviders(t *testing.T) {
	...
	skipWhenPaymentProvidersAbsent(t, center, PaymentProviderAlipay, PaymentProviderWeChat, "xunhupay-aggregate")
	registry := center.paymentRegistrySnapshot()
	if registry == nil {
...
func TestXunHuPayOfficialPackageIsPaymentPlugin(t *testing.T) {
	...
	skipWhenPaymentPluginsAbsent(t, center, "official-payment-xunhupay")
...
func TestZhiFuFMOfficialPackageIsPaymentPlugin(t *testing.T) {
	...
	skipWhenPaymentPluginsAbsent(t, center, "official-payment-zhifufm")
	var plugin PluginView
	for _, item := range center.list() {
