...
6. 生成前遵守现有模型目录和审批流程；失败时说明原因，不自动重复收费生成。`,
			...
			SortWeight: 900, OwnerUID: owner, EffectiveUser: seedEffectiveUser{Name: "衍图", UID: owner},
		},
		{
...
5. 成功后创建新图片节点并连回源图；保留原图和用户标注，不覆盖历史结果。`,
			Status: 1, CreateTime: created, UpdateTime: created, Source: 3, Tag: "creative",
			SortWeight: 890, OwnerUID: owner, EffectiveUser: seedEffectiveUser{Name: "衍图", UID: owner},
		},
		{
...
5. 生成、下载、持久化和审批全部复用宿主现有链路，不直接访问第三方 API。`,
			Status: 1, CreateTime: created, UpdateTime: created, Source: 3, Tag: "creative",
			SortWeight: 880, OwnerUID: owner, EffectiveUser: seedEffectiveUser{Name: "衍图", UID: owner},
		},
	}
}
