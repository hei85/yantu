<p align="center">
  <img src="web/public/logo.svg" width="88" alt="衍图 logo">
</p>

<h1 align="center">衍图</h1>
...
衍图是一个开源的 AI 影视与短剧创作工作台：用自由画布组织创作，用结构化工作流管理剧本、角色、场景和分镜，并通过统一的任务系统完成图片、视频、音频与文本生成。
...
- **管理与渠道**：系统渠道、逻辑模型、功能开关、对象存储、响应拦截和管理后台。
...
打开 <http://localhost:3000>。首次启动时创建管理员账号（仅首个账号可注册），并在设置中配置模型渠道。前端默认将 `/api` 代理到 `http://127.0.0.1:8080`；如需修改代理目标，可设置 `VITE_API_PROXY_TARGET`。
...
- 管理员账号只在数据库为空时可创建（首个账号自动成为管理员），请在受控网络完成首次初始化。
...
<p align="center">
  <img src="assets/wx.jpg" alt="衍图 微信交流群" width="100%">
</p>

## 许可证和上游

本项目采用 [MIT](LICENSE) 协议。衍图基于 [basketikun/infinite-canvas](https://github.com/basketikun/infinite-canvas) 的早期版本进行二次开发，上游作者和贡献者保留其对应代码的权利与署名。
