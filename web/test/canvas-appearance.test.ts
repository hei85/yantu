...
describe("canvas appearance follows the workspace theme", () => {
    test("new canvases follow the workspace theme and turn dark with it", () => {
        expect(autoCanvasAppearance()).toEqual({ mode: "auto" });
        expect(resolveCanvasAppearance(autoCanvasAppearance(), "dark")).toEqual({
            baseTheme: "dark",
            background: canvasThemes.dark.canvas.background,
            grid: canvasThemes.dark.canvas.line,
        });
        expect(resolveCanvasAppearance(autoCanvasAppearance(), "light").background).toBe(canvasThemes.light.canvas.background);
    });

    test("migrates legacy unpinned light/dark defaults to follow-theme", () => {
        expect(normalizeCanvasAppearance({ mode: "light" }, "dark")).toEqual({ mode: "auto" });
        expect(normalizeCanvasAppearance({ mode: "dark" }, "light")).toEqual({ mode: "auto" });
        expect(resolveCanvasAppearance({ mode: "dark" }, "light").baseTheme).toBe("light");
    });

    test("keeps canvases the user pinned to a fixed light or dark look", () => {
        expect(canvasAppearanceForTheme("light")).toEqual({ mode: "light", pinned: true });
        expect(normalizeCanvasAppearance({ mode: "light", pinned: true }, "dark")).toEqual({ mode: "light", pinned: true });
        expect(resolveCanvasAppearance({ mode: "dark", pinned: true }, "light")).toEqual({
            baseTheme: "dark",
            background: canvasThemes.dark.canvas.background,
            grid: canvasThemes.dark.canvas.line,
        });
    ...
    test("follow-theme keeps the interface theme while custom colours stay available", () => {
        const appearance = normalizeCanvasAppearance({ mode: "custom", custom: {
            baseTheme: "light",
        ...
        const followTheme = enterCustomCanvasAppearance(appearance, "light");
        expect(followTheme.mode).toBe("custom");
        expect(enterCustomCanvasAppearance({ mode: "auto", custom: appearance.custom }, "dark").custom).toEqual(appearance.custom);
        expect(resolveCanvasAppearance({ mode: "auto", custom: appearance.custom }, "dark").baseTheme).toBe("dark");
    });
});
