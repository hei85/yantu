...
// videoProtocol 用于把视频模型的请求协议钉住：不传时走按渠道地址推导的默认值，
// 传值时专门验证某个协议的请求形状（例如 Sora 式 multipart）。
function configForCatalog(catalog: ChannelModelCatalogItem[], input: Partial<AiConfig> = {}, videoProtocol?: string) {
    ...
    const modelCosts = mergeFetchedChannelModelCosts(channel, catalog);
    if (videoProtocol) {
        for (const cost of modelCosts) {
            if (cost.capability === "video") cost.protocol = videoProtocol;
        }
    }
    const configured = { ...channel, modelCosts };
...
describe("public channel model catalog", () => {
    ...
    test("maps Omni metadata to friendly video/NewAPI configuration with 8/10 seconds and both ratios", () => {
        ...
        // 非 OpenAI 官方的渠道按中转站处理，视频默认走 /v1/video/generations。
    ...
    test("defaults relay video models to the NewAPI video-generations protocol", () => {
        // 模型目录只返回模型名，视频协议必须按渠道地址兑底，否则中转站的视频会发到错误的接口。
        const relay = createModelChannel({ id: "relay", name: "Relay", baseUrl: "https://zh.example.dpdns.org", apiKey: "synthetic-test-key", apiFormat: "openai" });
        expect(mergeFetchedChannelModelCosts(relay, [{ id: "minimax_h3_z0903", modelType: "video" }])[0]?.protocol).toBe("newapi-channel-2");

        const openai = createModelChannel({ id: "openai", name: "OpenAI", baseUrl: "https://api.openai.com", apiKey: "synthetic-test-key", apiFormat: "openai" });
        expect(mergeFetchedChannelModelCosts(openai, [{ id: "sora-2", modelType: "video" }])[0]?.protocol).toBe("newapi");

        const autodl = createModelChannel({ id: "autodl", name: "AutoDL", baseUrl: "https://autodl.art", apiKey: "synthetic-test-key", apiFormat: "openai" });
        expect(mergeFetchedChannelModelCosts(autodl, [{ id: "minimax_h3_b99_001", modelType: "video" }])[0]?.protocol).toBe("autodl-comfyui");
    });
    ...
    test("omits resolution_name for Omni and for auto instead of inventing 720p", async () => {
        ...
        const omniConfig = configForCatalog([omniCatalog], { videoSeconds: "10", size: "16:9", vquality: "720" }, "newapi");
        ...
        const autoConfig = configForCatalog([declaredResolution], { videoSeconds: "8", size: "16:9", vquality: "auto" }, "newapi");
    ...
    test("sends a declared compatible HD resolution for a video capability", async () => {
        ...
        const config = configForCatalog([catalog], { videoSeconds: "8", size: "16:9", vquality: "1080" }, "newapi");
    ...
    test("preserves provider resolution enums with direction suffixes across Canvas display and requests", async () => {
        ...
        const config = configForCatalog([catalog], { videoSeconds: "8", size: "16:9", vquality: "768P竖" }, "newapi");
