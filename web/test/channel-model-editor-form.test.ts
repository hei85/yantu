...
test("admin panels mount inside settings instead of a separate admin site", async () => {
    ...
    const systemPane = await Bun.file(new URL("../src/pages/settings/system-settings-pane.tsx", import.meta.url)).text();
    ...
    // 旧后台地址全部回流到设置，不再渲染独立的 /admin 站点。
    expect(router).not.toContain("AdminPage");
    expect(router).toContain('path: "/admin/*"');
    expect(router).toContain('to="/settings?section=prompt-templates"');
    expect(router).toContain('path: "/admin/resources"');

    // 后台面板现在挂在设置分区下，并共用一个 Suspense 边界。
    expect(settings).toContain('"prompt-templates"');
    expect(settings).toContain("FeatureAvailabilityPane");
    // 存储资源与系统性能已按用户要求从系统分区下线。
    expect(settings).not.toContain("StorageResourcesPane");
    expect(settings).not.toContain("SystemPerformancePane");
    expect(systemPane).toMatch(/<Suspense[^]*?<\/Suspense>/);
