...
test("each channel becomes its own first-level group and preserves its own prices", () => {
    ...
    expect(groups.map((group) => [group.label, group.kind, group.models.map((item) => item.models)])).toEqual([
        ["正常渠道", "channel", [["a::seedance-2.0"]]],
        ["渠道 B", "channel", [["b::seedance-2.0"]]],
        ["渠道 C", "channel", [["c::seedance-2.0"]]],
    ]);
...
test("a channel keeps its own model key even when display names match across channels", () => {
    ...
    expect(groups.map((group) => group.models.flatMap((item) => item.models))).toEqual([["mita::MiniMax H3"], ["other::minimax-h3-alt"]]);
});
...
test("one channel lists all of its models as second-level entries", () => {
    const config = fixture();
    ...
    const single = { ...config, channels: extra };
    const groups = groupModelsForPicker(single, selectableModelsByCapability(single, "video"));
    expect(groups).toHaveLength(1);
    expect(groups[0].label).toBe("火山引擎");
    expect(groups[0].kind).toBe("channel");
    expect(groups[0].models.map((item) => item.models[0]).sort()).toEqual(["volc::seedance-2", "volc::seedance-2-fast", "volc::seedance-2-mini"]);
});
...
test("each channel keeps its own capability restrictions", () => {
    ...
    const groups = groupModelsForPicker(config, selectableModelsByCapability(config, "video"));
    expect(groups.flatMap((group) => group.models)).toHaveLength(3);
});
...
test("managed logical models and personal channels never become system product routes", () => {
    ...
    expect(groups.map((item) => item.kind)).toEqual(["channel", "channel", "channel"]);
