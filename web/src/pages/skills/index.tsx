    ...
    return (
        <article style={style} className={`product-collection-card library-card library-card-surface skill-library-card group${skill.isAdded ? " is-added" : ""}`}>
            ...
            <div className="skill-card-footer">
                ...
                {skill.isOwner ? <span className="skill-card-flag">我创建的</span> : null}
            ...
            {skill.isOwner
                ? (
                    <div className="skill-card-action">
                        {/* 自己创建的技能始终在技能库里，无法退出，所以这里只显示状态而不是加入按钮。 */}
                        <span className="skill-card-owner-state" title="这是你创建的技能，始终保留在「我的技能」中"><Check className="size-3.5" />已在技能库</span>
                        <Tooltip title={`${formatSkillCount(skill.addedCount)} 人已加入`}><span className="skill-card-added-count">{formatSkillCount(skill.addedCount)} 人已加入</span></Tooltip>
                    </div>
                )
                : (
                    <div className="skill-card-action">
