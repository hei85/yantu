...
import { Copy, ListChecks, Pencil, Plus, Power, Search, Trash2 } from "lucide-react";
...
export default function ChannelsPage() {
    ...
    // 拉取模型必须用已经落库的连接信息，否则后端会拿旧的 Base URL / Key 去请求上游。
    const saveThenManageModels = async () => {
        ...
        setSaving(true);
        try {
            await updateAdminChannel(editingChannel.id, adminChannelSavePayload(values));
            await syncChannels();
            setDrawerOpen(false);
            form.resetFields();
            await reload();
            setManagingChannel(editingChannel);
        } catch (error) {
            message.error(error instanceof Error ? error.message : "保存系统渠道失败");
        } finally {
            setSaving(false);
        ...
        >
                ...
                footer={
                    <div className="flex justify-end gap-2">
                        <Button
                            ...
                            disabled={!editingChannel || saving}
                            title={editingChannel ? "先保存连接信息，再拉取上游模型并挑选需要的那些" : "请先保存渠道，之后才能拉取模型"}
                            onClick={() => void saveThenManageModels()}
                        >
                            拉取 / 管理模型
                        </Button>
            ...
            >
                <Form form={form} layout="vertical" requiredMark={false}>
                    ...
                    <Form.Item name="baseUrl" label="Base URL" rules={[{ required: true, message: "请填写 Base URL" }]} extra="只填到域名，不要带 /v1、/v1/videos 这类接口路径；请求路径由模型协议自动拼接。">
