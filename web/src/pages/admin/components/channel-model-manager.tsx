...
import { App, Button, Checkbox, Input, Modal, Popconfirm, Radio, Select, Space } from "antd";
...
export function ChannelModelManager({ channel, onClose, onChanged, section, backLabel = "返回系统渠道", autoFetch = false }: { channel: ModelChannel; onClose: () => void; onChanged: () => void | Promise<void>; section?: { label: string; path: string }; backLabel?: string; autoFetch?: boolean }) {
    const { message, modal } = App.useApp();
    const [items, setItems] = useState<ChannelModel[]>([]);
    const [selectedModelIds, setSelectedModelIds] = useState<string[]>([]);
    const [deletingSelected, setDeletingSelected] = useState(false);
    // 批量配置：选中多个模型后一次改能力 / 协议 / 启用状态，不用逐个点“编辑”。
    const [batchConfigOpen, setBatchConfigOpen] = useState(false);
    const [batchCapability, setBatchCapability] = useState<"keep" | ChannelModel["capability"]>("keep");
    const [batchProtocol, setBatchProtocol] = useState<string>("keep");
    const [batchStatus, setBatchStatus] = useState<"keep" | "enable" | "disable">("enable");
    ...
    // 从「保存并拉取模型」进来时直接弹出可选模型清单；每个渠道只自动拉一次。
    const autoFetchedRef = useRef("");
    useEffect(() => {
        if (!autoFetch || autoFetchedRef.current === channel.id) return;
        autoFetchedRef.current = channel.id;
        void fetchModels();
        // fetchModels 每次渲染都是新引用，这里只按渠道变化触发。
    }, [autoFetch, channel.id]);
    ...
    // 批量应用能力 / 协议 / 启用状态：只改用户选中的项，其余字段原样保留。
    const applyBatchConfig = async () => {
        const selected = items.filter((item) => selectedModelIds.includes(item.id));
        if (!selected.length) return;
        if (batchCapability === "keep" && batchProtocol === "keep" && batchStatus === "keep") {
    ...
    // 批量配置弹窗里的协议选项：按选中的能力过滤，并附带真实请求行，避免选错协议。
    const batchProtocolOptions = [
        { value: "keep", label: "不修改" },
        ...availableProtocols
            .filter((protocol) => protocol.enabled !== false && (batchCapability === "keep" || protocol.capability === batchCapability))
            .map((protocol) => ({ value: protocol.value, label: `${protocol.label} · ${protocol.create.replace(/^POST /, "")}` })),
    ];
    ...
    return (
        ...
        >
            ...
            <AdminBatchBar count={selectedModelIds.length} onClear={() => setSelectedModelIds([])}>
                <Button size="small" icon={<SlidersHorizontal className="size-3.5" />} onClick={() => setBatchConfigOpen(true)}>
                    批量配置
                </Button>
            ...
            >
                <div className="space-y-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        <label className="grid gap-1.5 text-sm">
                            ...
                            <Select
                                ...
                                onChange={(value) => { setBatchCapability(value); setBatchProtocol("keep"); }}
                                options={[
                                    { value: "keep", label: "不修改" },
                                    { value: "text", label: "文本" },
                        ...
                        <label className="grid gap-1.5 text-sm">
                            <span className="text-foreground/65">请求协议</span>
                            <Select
                                showSearch
                    ...
                    <div className="grid gap-1.5 text-sm">
                        <span className="text-foreground/65">状态</span>
                        <Radio.Group
                            value={batchStatus}
