...
import { autoCanvasAppearance, canvasAppearanceBaseTheme, DEFAULT_CANVAS_BACKGROUND_MODE, normalizeCanvasAppearance, resolveCanvasAppearance, writeCanvasAppearanceDefault, type CanvasAppearance } from "@/lib/canvas/canvas-appearance";
...
function InfiniteCanvasPage() {
    ...
    // 画布默认跟随工作台主题，只有用户在「画布外观」里显式选过浅色/深色才固定下来。
    const [canvasAppearance, setCanvasAppearance] = useState<CanvasAppearance>(() => autoCanvasAppearance());
    const colorTheme = canvasAppearance.mode === "auto" ? workspaceTheme : canvasAppearanceBaseTheme(canvasAppearance, workspaceTheme);
    ...
    const resolvedCanvasAppearance = useMemo(() => resolveCanvasAppearance(canvasAppearance, workspaceTheme), [canvasAppearance, workspaceTheme]);
    const applyCanvasAppearance = useCallback(
        (next: CanvasAppearance) => {
            const normalized = normalizeCanvasAppearance(next, workspaceTheme);
            setCanvasAppearance(normalized);
            setTheme(canvasAppearanceBaseTheme(normalized, workspaceTheme));
        },
        [workspaceTheme, setTheme],
    );

    // 工作台切换深浅色时同步画布编辑器主题（含 body 上的 AntD 浮层）。
    useEffect(() => {
        setTheme(colorTheme);
    }, [colorTheme, setTheme]);
            ...
            <CanvasShortDramaEmptyState
                onCreatePipeline={createShortDramaPipeline}
                    onOpenAgent={() => {
                        ...
                        openAgent();
                        // 这里只是状态卡片，不是聊天窗口：重开时给句提示，避免“点了没反应”的误解。
                        message.info("本地 Canvas Agent 状态已在右下角显示；真正的对话在 Codex 里 —— 用 yingce MCP 就能读写这个画布。", 6);
                    }}
