...
export function CreationAgentEntry() {
    ...
    // 网络或后端异常时，同步步骤可能一直不返回；没有超时会让 lock 永久占住，按钮从此点不动。
    const withCreationTimeout = <T,>(task: Promise<T>, timeoutMs = 20_000) =>
        Promise.race<T>([
            task,
            new Promise<never>((_, reject) => window.setTimeout(() => reject(new Error("创建画布超时，请重试。")), timeoutMs)),
        ]);
    const start = async () => {
        if (lock.current) {
        ...
        try {
            ...
            if (!created.current) {
                const result = await withCreationTimeout(createCanvasProjectWithRemoteSync("Agent 创作"));
                ...
                await withCreationTimeout(saveRemoteUserDataNow());
            }
