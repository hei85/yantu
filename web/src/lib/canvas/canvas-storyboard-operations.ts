...
export function validateAgentStoryboardCreation(ops: CanvasOperation[]) {
    for (const op of ops) {
        ...
        const oversize = rows.filter((row) => Number.isFinite(row.durationSeconds) && row.durationSeconds > MAX_SHOT_DURATION_SECONDS);
        if (oversize.length) throw new Error(`分镜单镜时长不能超过 ${MAX_SHOT_DURATION_SECONDS} 秒（视频模型上限，超时长的镜头无法生成视频）：第 ${oversize.map((row) => row.shotNumber ?? "?").join("、")} 镜为 ${oversize.map((row) => row.durationSeconds).join("、")} 秒，请拆成两镜或压缩后重写分镜`);
    }
    return ops;
}
    ...
    if (action === "remove") {
        ...
        for (const [key, value] of Object.entries(data)) {
            if (key === "durationSeconds") {
                ...
                if (value > MAX_SHOT_DURATION_SECONDS) throw new Error(`单镜时长不能超过 ${MAX_SHOT_DURATION_SECONDS} 秒（视频模型上限）：请拆成两镜或压缩到 ${MAX_SHOT_DURATION_SECONDS} 秒以内`);
