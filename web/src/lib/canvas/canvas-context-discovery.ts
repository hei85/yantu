...
// 视频模型单镜硬上限：超过 15 秒的镜头无法生成视频，分镜写入时直接拦下。
...
function validateStoryboardShotDurations(issues: Array<{ index: number; severity: "error" | "warning"; message: string }>, index: number, metadata: unknown, label: string) {
    ...
    const rows = (storyboard as Record<string, unknown>).rows;
...
export function validateCanvasOperations(snapshot: CanvasSnapshot, ops: CanvasOperation[]) {
    ...
    ops.forEach((op, index) => {
        if (op.type === "add_node") {
            ...
            validateStoryboardShotDurations(issues, index, op.metadata, "新增分镜节点");
            ...
            validateStoryboardShotDurations(issues, index, metadata, "更新分镜节点");
            validateStoryboardShotDurations(issues, index, patchMetadata, "更新分镜节点");
