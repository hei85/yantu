...
export type CanvasAppearanceMode = CanvasColorTheme | "custom" | "auto";
...
export type CanvasAppearance = {
    ...
    // 用户在「画布外观」里显式选过浅色/深色；缺省表示跟随工作台主题。
    pinned?: boolean;
};
...
// 新建画布默认跟随工作台主题：工作台切深色时画布一起变深色。
export function autoCanvasAppearance(): CanvasAppearance {
    return { mode: "auto" };
}

// 显式指定浅色/深色；带 pinned 标记，避免被历史默认值迁移成 auto。
export function canvasAppearanceForTheme(theme: CanvasColorTheme, previous?: CanvasAppearance): CanvasAppearance {
    const custom = previous?.custom?.baseTheme === theme ? previous.custom : undefined;
    return custom ? { mode: theme, pinned: true, custom } : { mode: theme, pinned: true };
}
...
export function canvasAppearanceBaseTheme(appearance: CanvasAppearance | undefined, fallback: CanvasColorTheme): CanvasColorTheme {
    if (appearance?.mode === "auto") return fallback;
...
export function normalizeCanvasAppearance(value: unknown, fallback: CanvasColorTheme): CanvasAppearance {
    if (!value || typeof value !== "object") return autoCanvasAppearance();
    const candidate = value as Partial<CanvasAppearance>;
    const custom = normalizeCustomAppearance(candidate.custom);
    if (candidate.mode === "custom") return custom ? { mode: "custom", custom } : customCanvasAppearanceFromTheme(fallback);
    if (candidate.mode === "auto") return custom ? { mode: "auto", custom } : autoCanvasAppearance();
    if (candidate.mode !== "light" && candidate.mode !== "dark") return custom ? { mode: "auto", custom } : autoCanvasAppearance();
    // 历史数据里没有 pinned 的浅色/深色只是旧默认值，迁移为跟随工作台主题。
    if (candidate.pinned !== true) return custom ? { mode: "auto", custom } : autoCanvasAppearance();
    return custom ? { mode: candidate.mode, pinned: true, custom } : { mode: candidate.mode, pinned: true };
}
