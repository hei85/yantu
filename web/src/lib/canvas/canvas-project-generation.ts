...
export function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode, requirements?: ModelRequirements): AiConfig {
    ...
    // 视频可选开关必须以当前模型的能力档案为准：全局默认（同步音频默认开启）直接发出去，
    // 会被后端合同拒绝，报“参数 同步音频 超出支持范围”。
    const videoProfile = modelCapabilityConfigFor(config, model).video;
    const videoGenerateAudio = videoProfile && !videoProfile.generateAudio.supported
        ? String(videoProfile.generateAudio.default)
        : generationDefaults.videoGenerateAudio || requestedConfig.videoGenerateAudio;
    const videoWatermark = videoProfile && !videoProfile.watermark.supported
        ? String(videoProfile.watermark.default)
        : generationDefaults.videoWatermark || requestedConfig.videoWatermark;
